document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- State Variables ---
    let isTyping = false; let randomPopupCount = 0; const MAX_RANDOM_POPUPS = 2; let potentialPopupMessages = ["Default message 1...", "Default message 2..."]; let fakeProgressInterval = null; const ESTIMATED_PROCESSING_TIME_MS = 15000;
    ...
    function showProgress() {
        ...
        fakeProgressInterval = setInterval(() => {
            ...
            console.log(`DEBUG Interval: Elapsed=${elapsedTime}ms, Progress=${progressPercent.toFixed(1)}%, Display=${displayPercent.toFixed(1)}%`); // Log Progress
    ...
    function showResult(fileBlob, filename) {
        ...
        fetchProcessingSummary(); // Fetch and show summary popup
        const aiSpeaker = 'Bingus ✨'; displayMessage(aiSpeaker, "YAS QUEEN! 👑 File processed! Download below & check report!");
    ...
    async function sendChatMessage() {
        console.log("DEBUG: sendChatMessage function CALLED!"); // Log Entry
        const message = chatInput?.value?.trim(); console.log(`DEBUG: Message to send: "${message}"`);
        if (!message) { console.log("DEBUG: Message empty."); return; } if (!chatInput) { console.error("ERROR: chatInput null!"); return; }
        displayMessage('You', message); // Display user message first
        chatInput.value = ''; chatInput.focus(); showTypingIndicator(); console.log(`DEBUG: Attempting to fetch /chat`);
        try {
            ...
            const responseText = await response.text(); console.log(`DEBUG: /chat response text: ${responseText}`); // Log raw text
            ...
            const data = JSON.parse(responseText); console.log("DEBUG: Parsed /chat data:", data);
            hideTypingIndicator(); const aiSpeaker = 'Bingus ✨';
    ...
    // --- Random Pop-up Logic ---
    async function fetchRandomMessages() { /* ... (keep existing) ... */ }
    function showRandomPopup(messageContent = null) { /* ... (keep existing) ... */ }
    function hideRandomPopup() { /* ... (keep existing) ... */ }

    // --- Summary Pop-up Logic ---
    async function fetchProcessingSummary() {
        console.log("DEBUG: Fetching processing summary...");
        try {
            const response = await fetch('/get-processing-summary'); if (!response.ok) { throw new Error(`HTTP error! status: ${response.status}`); }
            const summaryData = await response.json(); console.log("DEBUG: Received summary data:", summaryData);
            if (summaryData && Object.keys(summaryData).length > 0) { showSummaryPopup(summaryData); }
            else { console.log("WARN: No summary data received."); displayMessage('Bingus ✨', "File ready, but summary details lost!"); }
        } catch (error) { console.error("ERROR: Could not fetch summary:", error); displayMessage('Bingus ✨', "Couldn't fetch final report card!"); }
    }
    function showSummaryPopup(data) {
        if (!summaryPopupOverlay || !summaryPopupModal || !summaryPopupContent) { console.error("ERROR: Summary popup elements missing!"); return; }
        let contentHtml = `
        ...
        console.log("DEBUG: Summary popup shown.");
    }
    function hideSummaryPopup() {
        if (!summaryPopupOverlay || !summaryPopupModal) return;
        summaryPopupOverlay.classList.add('animate-fade-out'); summaryPopupModal.classList.add('animate-scale-out');
        setTimeout(() => {
            summaryPopupOverlay.classList.add('hidden'); summaryPopupOverlay.classList.remove('opacity-100', 'animate-fade-in', 'animate-fade-out'); summaryPopupOverlay.classList.add('opacity-0');
            summaryPopupModal.classList.remove('opacity-100', 'scale-100', 'animate-scale-in', 'animate-scale-out'); summaryPopupModal.classList.add('opacity-0', 'scale-95');
        }, 300); console.log("DEBUG: Summary popup hidden.");
    }
    ...
    // --- Initial Load ---
    try {
        console.log("DEBUG: Running initial setup..."); clearChatLoading(); fetchRandomMessages().then(() => { setTimeout(showRandomPopup, 4000); setTimeout(showRandomPopup, 45000); });
        setTimeout(() => { console.log("DEBUG: Calling displayMessage for initial greeting."); displayMessage('Bingus ✨', "Heeey gorgeous! Welcome! Upload a file or ask me anything! 💖"); }, 1000);
