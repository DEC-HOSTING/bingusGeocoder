...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "default_unsafe_dev_key_CHANGE_ME") # Ensure this is set in .env
...
def find_columns(df):
    ...
    if not address_col or not postcode_col:
        missing = [col for col, name in [(address_col,"Address/Street"),(postcode_col,"Postcode/Postal/Zip")] if not col]
        raise ValueError(f"Oh no, hun! 💅 Couldn't find columns like '{ ' or '.join([name for _, name in missing]) }'. Check headers!")
...
def get_coords(address, postcode, row_num):
    ...
    try:
        location = geocode(query_input, language='en', timeout=15) # Optional: country_codes='ES'
        print(f"DEBUG (Row {row_num}): Nominatim returned: {location}")
        if location and hasattr(location, 'latitude') and location.latitude is not None and hasattr(location, 'longitude') and location.longitude is not None: # Safe attribute checking
...
def talk_to_bingus(prompt, conversation_history=[], model_name="google/gemma-3-27b-it", max_resp_tokens=180):
    ...
    print(f"DEBUG: talk_to_bingus called with prompt: '{prompt[:60]}...'") # Log Entry
    ...
    system_prompt = ("You are Bingus...") # Keep full system prompt
    messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}] # Add history if needed
    try:
        print(f"DEBUG: Calling ai_client.chat.completions.create (Model: {model_name}, MaxTokens: {max_resp_tokens})") # Log API Call
        ...
        print(f"DEBUG: Raw completion object received: {completion}") # Log Raw Response
    ...
    except AuthenticationError as e: print(f"🚨 FATAL: Kluster AI Auth Failed! Check Key. Err: {e}"); traceback.print_exc(); return "OMG DRAMA! 😱 AI Auth Error!"
    ...
    except Exception as e: print(f"🚨 ERROR: Unexpected error in talk_to_bingus: {e}"); traceback.print_exc(); return f"Yikes, technical difficulties! Err: {e}"
...
def process_excel():
    ...
    try:
        ...
        # Optional TEXT conversion commented out
        ...
        # Save & Send File
        output=io.BytesIO();
        with pd.ExcelWriter(output, engine='openpyxl') as writer: df.to_excel(writer, index=False, sheet_name="Processed_Data"[:31].replace('/','-'), na_rep='') # Simplified sheet name cleaning
...
# --- Chat Route ---
# ✨ ENSURE THIS DECORATOR IS CORRECT ✨
@app.route('/chat', methods=['POST'])
def chat():
    """Handles user messages to the AI."""
    print("DEBUG: /chat route handler CALLED") # Log route entry
    if not ai_client:
        print("WARN: /chat - AI client not available.")
        return jsonify({"response": "Bingus is napping..."}), 503 # Service Unavailable might be better status
    user_message = request.json.get('message')
    if not user_message:
        print("WARN: /chat - No message in request body.")
    ...
    # Check if AI call itself returned an error message
    if "error" in ai_response.lower() or "failed" in ai_response.lower():
        # Maybe return a server error status if the AI call failed internally
        return jsonify({"response": ai_response}), 500 # Or keep 200 OK but show error in chat? Let's keep 200 for now.
...
def get_random_messages():
    ...
    default_messages=["Default Msg 1...", "Default Msg 2..."] # Keep brief
    ...
    try:
        ...
        base_prompts=["Quick compliment!","Tiny sassy fact?","Encouraging words!","Chic fashion tip?","Sparkle reminder!","Madrid time comment!"] # Shorter
...
if __name__ == '__main__':
    ...
    print(f"Secret Key Loaded: {'Yes' if os.getenv('FLASK_SECRET_KEY') != 'default_unsafe_dev_key_CHANGE_ME' else 'No (Using default - UNSAFE!)'}")
